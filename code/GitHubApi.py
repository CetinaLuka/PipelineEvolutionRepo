...
class GitHubApi:
    ...
    def get_jobs_in_workflow(self, yaml_content):
        """Parse the YAML content of multiple workflows to list all jobs and check for specific phases."""
        jobs = []
        phases = {'test': False, 'build': False, 'analyze': False, 'release': False, 'deploy': False}
        try:
            # Load all YAML documents
            workflow_yamls = yaml.safe_load_all(yaml_content)

            # Process each document
            for workflow_yaml in workflow_yamls:
                if workflow_yaml is None:
                    continue
                for job_name, job_details in workflow_yaml.get('jobs', {}).items():
                    jobs.append(job_name)
                    self.check_job_for_phases(job_name, job_details, phases)
        except yaml.YAMLError as e:
            print(f"Error parsing YAML: {e}")
        return jobs, phases

    def check_job_for_phases(self, job_name, job_details, phases):
        """Check if job names or commands in steps suggest specific development phases."""
        # Common patterns to check in job names and commands
        phase_keywords = {
            'test': ['test', 'unittest', 'mvn test', 'gradle test'],
            'build': ['build', 'compile', 'mvn package', 'mvn install', 'gradle build'],
            'release': ['release', 'publish', 'package', 'mvn deploy'],
            'analyze': ['analyze', 'analysis', 'lint', 'checkstyle', 'spotbugs', 'findbugs', 'sonar'],
            'deploy': ['deploy', 'deployment', 'mvn deploy']
        }
        ...
        for phase, keywords in phase_keywords.items():
            if any(keyword in job_name_lower for keyword in keywords):
                phases[phase] = True
                continue  # If found in job name, no need to check steps

            # Checking in steps if defined
            for step in job_details.get('steps', []):
                if 'run' in step:
                    commands = step['run']
                    if any(keyword in commands.lower() for keyword in keywords):
                        phases[phase] = True
    ...
    def check_repos_for_github_actions(self, repo_list_csv_path="data/all_repos.csv",
        ...
        for index, repo in df_repos.iterrows():
            ...
            jobs, phases = self.get_jobs_in_workflow(yaml_files_content)
            number_of_jobs = len(jobs)
            print(f"Number of jobs: {number_of_jobs}")
            print(f"Number of workflows: {number_of_workflows}")
            # Update the row in the DataFrame
            new_row = {**repo,
                       'has_pipeline': has_pipeline,
                       'num_of_workflows': number_of_workflows,
                       'num_of_jobs': number_of_jobs,
                       'jobs': jobs, 'yaml_files_content': yaml_files_content,
                       'workflows': workflows,
                       'test': phases['test'],
                       'build': phases['build'],
